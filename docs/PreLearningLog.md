...
- 개념 이해
  - 객체의 상태를 직접 가져오는(getter) 대신,
  - 객체에 메시지를 보내 해당 객체가 스스로 행동하도록 하는 방식을 선호함
  - 이는 캡슐화를 강화하고 객체 간의 결합도를 낮추는 데 도움이 됨
- 특징
  - 캡슐화 강화: 객체의 상태 정보를 직접 노출하지 않고, 객체에 정의된 메서드를 통해 상호작용함
  - 객체 자율성: 객체는 외부의 간섭 없이 스스로의 상태를 관리하고, 행동을 결정함
  - 유지보수 용이성: 객체의 내부 구현을 변경해도 외부에 미치는 영향이 적어 유지보수가 용이함
- 예시
  - 잔액 조회: 계좌 객체에 getBalance() 대신 showBalance() 메서드를 호출하여 잔액을 조회
  - 객체가 잔액을 스스로 계산하고 표시함